...
async fn main() -> Result<()> {
    ...
    match cfg.transport {
        McpTransport::Http => {
            ...
            info!(addr = %cfg.http_bind, "mcp-analytics HTTP server listening");
            axum::serve(listener, app)
                .await
                .context("mcp-analytics HTTP server terminated with error")?;
        }
        McpTransport::Stdio => {
            info!("Using stdio transport");
            let server = AnalyticsServer::new(pool);
            let transport = rmcp::transport::stdio();
            rmcp::serve_server(server, transport)
                .await
                .context("mcp-analytics server terminated with error")?;
        }
        McpTransport::Sse => {
    ...
    info!("mcp-analytics server shut down");
    Ok(())
}
