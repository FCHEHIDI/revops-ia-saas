...
use crate::server::BillingServer;
...
async fn main() -> Result<()> {
    ...
    match cfg.transport {
        McpTransport::Http => {
            ...
            info!(addr = %cfg.http_bind, "mcp-billing HTTP server listening");
            axum::serve(listener, app)
                .await
                .context("mcp-billing HTTP server terminated with error")?;
        }
        McpTransport::Stdio => {
            info!("Using stdio transport");
            let server = BillingServer::new(pool);
            let transport = rmcp::transport::stdio();
            rmcp::serve_server(server, transport)
                .await
                .context("mcp-billing server terminated with error")?;
        }
        McpTransport::Sse => {
    ...
    info!("mcp-billing server shut down");
    Ok(())
}
